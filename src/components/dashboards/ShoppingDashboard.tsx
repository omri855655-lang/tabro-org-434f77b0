...
const ShoppingDashboard = () => {
  ...
  const addItem = async () => {
    ...
    toast.success(isDream ? "נוסף לרשימת חלומות" : "נוסף לרשימת קניות");
    fetchItems();
  };
  ...
  const sendAiMessage = async () => {
    ...
    try {
      ...
      const shoppingItems = items.filter(i => !i.is_dream);
      const context = `רשימת קניות: ${shoppingItems.map(i => `${i.title} (${i.status})`).join(", ")}. רשימת חלומות: ${dreamItems.map(i => `${i.title} (מחיר: ${i.price || "לא ידוע"})`).join(", ")}.`;

      const { data, error } = await supabase.functions.invoke("task-ai-helper", {
        body: {
          taskDescription: aiChat,
          taskCategory: "shopping",
          customPrompt: `אתה יועץ קניות חכם. עזור למשתמש עם רשימת הקניות והחלומות שלו. ${context}\n\nהמשתמש שואל: ${aiChat}`,
        },
      });
  ...
  return (
    <div className="p-4 space-y-4 max-w-4xl mx-auto" dir="rtl">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h2 className="text-2xl font-bold">קניות וחלומות</h2>
      </div>

      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="w-full">
          <TabsTrigger value="shopping" className="flex-1 gap-2"><ShoppingCart className="h-4 w-4" />רשימת קניות ({shoppingItems.length})</TabsTrigger>
          <TabsTrigger value="dreams" className="flex-1 gap-2"><Star className="h-4 w-4" />רשימת חלומות ({dreamItems.length})</TabsTrigger>
          <TabsTrigger value="ai" className="flex-1 gap-2"><Sparkles className="h-4 w-4" />יועץ AI</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="shopping" className="space-y-4">
          ...
          <Card>
            <CardContent className="pt-4">
              <div className="flex gap-2 flex-wrap">
                <Input placeholder="מוצר חדש..." value={newTitle} onChange={e => setNewTitle(e.target.value)} onKeyDown={e => e.key === "Enter" && addItem()} className="flex-1 min-w-[150px]" />
                <Input placeholder="קטגוריה" value={newCategory} onChange={e => setNewCategory(e.target.value)} className="w-28" />
                ...
                <Button onClick={() => { setIsDream(false); addItem(); }} size="icon"><Plus className="h-4 w-4" /></Button>
              </div>
            </CardContent>
          </Card>
          ...
          {/* Group by category */}
          {(() => {
            const cats = [...new Set(shoppingItems.map(i => i.category || "כללי"))];
            return cats.map(cat => {
              const catItems = shoppingItems.filter(i => (i.category || "כללי") === cat);
              return (
                <Card key={cat}>
        ...
        <TabsContent value="dreams" className="space-y-4">
          <Card>
            <CardContent className="pt-4">
              <div className="flex gap-2 flex-wrap">
                <Input placeholder="חלום חדש (מוצר/חפץ)..." value={newTitle} onChange={e => setNewTitle(e.target.value)} onKeyDown={e => e.key === "Enter" && (() => { setIsDream(true); addItem(); })()} className="flex-1 min-w-[150px]" />
                <Input placeholder="קטגוריה" value={newCategory} onChange={e => setNewCategory(e.target.value)} className="w-28" />
                <Input placeholder="מחיר" type="number" value={newPrice} onChange={e => setNewPrice(e.target.value)} className="w-24" dir="ltr" />
                <Button onClick={() => { setIsDream(true); addItem(); }} size="icon" variant="secondary"><Star className="h-4 w-4" /></Button>
              </div>
            </CardContent>
          </Card>
