    ...
    <div className="p-6 space-y-6" dir="rtl">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            דשבורד אופציונלי לפגישות, תמלולים, סיכומי שיחה וייצוא ל‑PDF.
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent className="text-3xl font-bold">{meetings.length}</CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="text-base">עם סיכום AI</CardTitle></CardHeader>
          <CardContent className="text-3xl font-bold">{meetings.filter((m) => m.aiSummary.trim()).length}</CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="text-base">עם תמלול</CardTitle></CardHeader>
          <CardContent className="text-3xl font-bold">{meetings.filter((m) => m.transcript.trim()).length}</CardContent>
        </Card>
      </div>
            ...
            <Card key={meeting.id}>
              <CardContent className="py-4 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div className="space-y-1">
                  ...
                  <div className="text-sm line-clamp-2">{meeting.shortSummary || meeting.aiSummary || "אין עדיין סיכום."}</div>
                </div>
