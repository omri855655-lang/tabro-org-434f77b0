...
export function ZoneFlowTogetherStudio({ isLight }: { isLight: boolean }) {
  ...
  return <div className="space-y-4" dir={dir}>
    ...
    {tab === "competitions" && <div className="grid gap-4 xl:grid-cols-3">{competitionCards.map((competition) => { const Icon = competition.icon; return <Card key={competition.id} className={cn("border", panel)}><CardHeader><CardTitle className="flex items-center gap-2"><Icon className="h-5 w-5 text-amber-500" />{competition.title}</CardTitle></CardHeader><CardContent><p className={cn("text-sm leading-6", muted)}>{competition.detail}</p><div className="mt-5 text-2xl font-bold">{competition.value}</div><div className={cn("mt-1 text-xs", muted)}>{copy.rank}: #12 · {copy.username}: {username}</div><Progress value={competition.id === "books" ? Math.min(100, totalPages / 5) : competition.id === "focus" ? Math.min(100, focusMinutes / 3) : Math.min(100, unlockMinutes)} className="mt-4 h-2" /></CardContent></Card> })}</div>}
