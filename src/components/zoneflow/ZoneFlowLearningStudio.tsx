...
const CATEGORIES = ["פיזיקה", "היסטוריה", "כימיה", "ביולוגיה", "פילוסופיה", "כלכלה", "פסיכולוגיה", "טכנולוגיה"];
...
interface LearningEntry { id: string; date: string; category: string; title: string; summary: string; feedback?: string }
...
export function ZoneFlowLearningStudio({ userId }: { userId?: string }) {
  ...
  const generateTopic = async (requestedCategory = category) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("task-ai-helper", {
        body: {
          type: "custom",
          prompt: `צור שיעור יומי קצר בעברית בתחום ${selected}. החזר בדיוק בפורמט JSON עם title ו-content. התוכן צריך להתאים לכ-5 דקות קריאה, להיות מדויק, ברור, ללא Markdown, ולכלול רעיון מרכזי, דוגמה ושאלת מחשבה. אל תיתן ייעוץ רפואי או פיננסי אישי.`,
        },
      });
      ...
      setTopic(FALLBACK_TOPICS[selected] || { title: `מבוא קצר ל${selected}`, content: `${selected} הוא תחום רחב שמנסה להסביר תופעות, רעיונות ודפוסים. בשיעור הראשון כדאי לבחור מושג בסיסי, לתאר אותו במילים פשוטות, למצוא דוגמה מחיי היום־יום ולנסח שאלה אחת שנרצה לחקור בהמשך. נסו לחשוב: איזה חלק בתחום הזה כבר פוגש אתכם ביום רגיל?` });
  ...
  const completeLesson = async () => {
    ...
    const entry = { id: crypto.randomUUID(), date: new Date().toISOString(), category, title: topic.title, summary: summary.trim(), feedback: nextFeedback };
      ...
      <section className="overflow-hidden rounded-[2rem] border border-[#dbcda9] bg-[#fffdf7] shadow-[0_24px_70px_rgba(80,63,35,.12)]">
        <div className="grid gap-6 p-5 lg:grid-cols-[.7fr_1.3fr] sm:p-7">
          <div>
            ...
            <div className="mt-5 flex flex-wrap gap-2">{CATEGORIES.map((item) => <button key={item} onClick={() => setCategory(item)} className={`rounded-full border px-3 py-1.5 text-xs transition ${category === item ? "border-amber-700 bg-amber-700 text-white" : "border-[#dfd5bd] bg-white hover:bg-amber-50"}`}>{item}</button>)}</div>
